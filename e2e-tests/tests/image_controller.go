		...
		When("a new component is created with visibility private", Label("private"), func() {
			...
			It("credential rotaion is successful", func() {
				...
				Eventually(func() error {
					...
					return nil
				}, 30*time.Second, 2*time.Second).Should(Succeed(), fmt.Sprintf("timed out when checking generate timestamp is updated for %s/%s", testNamespace, imageRepositoryCRName))
			})
		})
