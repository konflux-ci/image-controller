...
func (r *ApplicationReconciler) getComponentIdsForApplication(ctx context.Context, applicationId types.UID, namespace string) ([]types.UID, error) {
...
func (r *ApplicationReconciler) getImageRepositoryPullSecretNamesForComponents(ctx context.Context, componentIds []types.UID, namespaceName string) ([]string, error) {
	log := ctrllog.FromContext(ctx)
...
func (r *ApplicationReconciler) createApplicationServiceAccount(ctx context.Context, serviceAccountName string, application *appstudioredhatcomv1alpha1.Application, pullSecretNames []string) error {
	...
	pullSecretReferences := []corev1.ObjectReference{}
	for _, secretName := range pullSecretNames {
		pullSecretReferences = append(pullSecretReferences, corev1.ObjectReference{Name: secretName})
	}
	...
	return r.createApplicationServiceAccount(ctx, serviceAccountName, application, pullSecretNames)
}
