...
func (r *ImageRepositoryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if isComponentLinked(imageRepository) {
		...
		if err := r.linkSecretToServiceAccount(ctx, applicationSaName, pullSecretName, imageRepository.Namespace); err != nil {
		...
		componentSaName := getComponentSaName(imageRepository.Labels[ComponentNameLabelName])
		if err := r.linkSecretToServiceAccount(ctx, componentSaName, pushSecretName, imageRepository.Namespace); err != nil {
...
func (r *ImageRepositoryReconciler) EnsureSecret(ctx context.Context, imageRepository *imagerepositoryv1alpha1.ImageRepository, secretName string, robotAccount *quay.RobotAccount, imageURL string, isPull bool) error {
	...
	if !isPull {
		if err := r.linkSecretToServiceAccount(ctx, buildPipelineServiceAccountName, secretName, imageRepository.Namespace); err != nil {
...
func (r *ImageRepositoryReconciler) linkSecretToServiceAccount(ctx context.Context, saName, secretNameToAdd, namespace string) error {
	...
	secret := &corev1.Secret{}
	secretKey := types.NamespacedName{Namespace: namespace, Name: secretNameToAdd}
	if err := r.Client.Get(ctx, secretKey, secret); err != nil {
		if !errors.IsNotFound(err) {
			log.Error(err, "failed to get secret", l.Action, l.ActionView)
			return err
		}
		// secret doesn't exist don't link it to SA
		return nil
	}
...
func (r *ImageRepositoryReconciler) cleanUpSecretInServiceAccount(ctx context.Context, saName, secretName, namespace string) error {
...
func (r *ImageRepositoryReconciler) VerifyAndFixSecretsLinking(ctx context.Context, imageRepository *imagerepositoryv1alpha1.ImageRepository) error {
	...
	pushSecretKey := types.NamespacedName{Namespace: imageRepository.Namespace, Name: pushSecretName}
	pushSecretExists := true
	if err := r.Client.Get(ctx, pushSecretKey, pushSecret); err != nil {
		if !errors.IsNotFound(err) {
			log.Error(err, "failed to get secret", "SecretName", pushSecretName, l.Action, l.ActionView)
			return err
		}

		pushSecretExists = false
		log.Info("Secret doesn't exist, will unlink secret from service account", "SecretName", pushSecretName)

		if err := r.unlinkSecretFromServiceAccount(ctx, buildPipelineServiceAccountName, pushSecretName, imageRepository.Namespace); err != nil {
			log.Error(err, "failed to unlink secret from service account", "saName", buildPipelineServiceAccountName, "SecretName", pushSecretName, l.Action, l.ActionUpdate)
			return err
		}
		if isComponentLinked(imageRepository) {
	...
	if pushSecretExists {
		...
		if err := r.linkSecretToServiceAccount(ctx, buildPipelineServiceAccountName, pushSecretName, imageRepository.Namespace); err != nil {
		...
		if err := r.cleanUpSecretInServiceAccount(ctx, buildPipelineServiceAccountName, pushSecretName, imageRepository.Namespace); err != nil {
		...
		if isComponentLinked(imageRepository) {
			// link secret to service account if isn't linked already
			if err := r.cleanUpSecretInServiceAccount(ctx, componentSaName, pushSecretName, imageRepository.Namespace); err != nil {
	...
	if isComponentLinked(imageRepository) {
		...
		pullSecretKey := types.NamespacedName{Namespace: imageRepository.Namespace, Name: pullSecretName}
		pullSecretExists := true
		if err := r.Client.Get(ctx, pullSecretKey, pullSecret); err != nil {
			if !errors.IsNotFound(err) {
				log.Error(err, "failed to get secret", "SecretName", pullSecretName, l.Action, l.ActionView)
				return err
			}

			pullSecretExists = false
			log.Info("Secret doesn't exist, will unlink secret from service account", "SecretName", pullSecretName)

			if err := r.unlinkSecretFromServiceAccount(ctx, applicationSaName, pullSecretName, imageRepository.Namespace); err != nil {
				log.Error(err, "failed to unlink secret from service account", "saName", applicationSaName, "SecretName", pullSecretName, l.Action, l.ActionUpdate)
				return err
			}
		}
		...
		if pullSecretExists {
			...
			// clean duplicate secret links and remove secret from ImagePullSecrets
			if err := r.cleanUpSecretInServiceAccount(ctx, applicationSaName, pullSecretName, imageRepository.Namespace); err != nil {
